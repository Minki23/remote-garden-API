...
class UserDeviceRepository(SuperRepo[UserDeviceDb]):
    """
    Repository for managing user devices (FCM tokens, push notification targets).
    """
    ...
    async def get_by_fcm_token(self, token: str) -> Optional[UserDeviceDb]:
        """
        Return a user device by its FCM token, or None if not found.
        """
    ...
    async def get_by_user_id(self, user_id: int) -> list[UserDeviceDb]:
        """
        Return all user devices belonging to a given user.
        """
        result = await self.db.execute(
